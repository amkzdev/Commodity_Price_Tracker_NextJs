...
export const SideBar = () => {
    ...
    const { data, isLoading, commodity, period, currency } = useChartData()
    ...
    if (isLoading)
        ...
        const allData: number[] = data?.data.map(i => i.value)


        const lowestPrice = allData?.sort()?.[0].toFixed(2)
        const highestPrice = allData?.sort().toReversed()?.[0].toFixed(2)
